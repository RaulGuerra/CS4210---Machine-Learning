...
for w in n: #iterates over n
    ...
    for b in r: #iterates over r
        ...
        for a in range(2): #iterates over the algorithms
            ...
            #print("accuratePredictions: " + str(accuratePredictions) + ", totalPredictions: " + str(totalPredictions)) #-------------FOR TESTING ONLY----------------------------
            for (x_testSample, y_testSample) in zip(X_test, y_test):
            ...
            #--> add your Python code here
            if a==0: #PERCEPTRON OUTPUT
                #print(str(accuratePredictions / totalPredictions)) #-------------FOR TESTING ONLY----------------------------
                if (accuratePredictions / totalPredictions) > perceptronAccuracy:
                    perceptronAccuracy = accuratePredictions / totalPredictions
                    print("Highest Perceptron accuracy so far: " + str(perceptronAccuracy) + ", Parameters: learning rate=" + str(w) + ", shuffle=" + str(b))

            else: #MLP OUTPUT
                #print(str(accuratePredictions / totalPredictions)) #-------------FOR TESTING ONLY----------------------------
                if (accuratePredictions / totalPredictions) > mlpAccuracy:
                    mlpAccuracy = accuratePredictions / totalPredictions
                    print("Highest MLP accuracy so far: " + str(mlpAccuracy) + ", Parameters: learning rate=" + str(w) + ", shuffle=" + str(b))
